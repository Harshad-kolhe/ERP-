...
/// cell makes the operator fix one thing, re-upload, and discover the next â€” a
...
/// <see cref="ImportSheetBinder.RequireColumns"/> â€” a missing required column is a
...
public sealed class ImportRowReader(ExcelRow row)
{
    ...
    /// Returns empty rather than null when missing â€” the error is already recorded,
    ...
    /// <para>
    /// For a caller that needs to look at a column a <em>second</em> time â€” checking
    ...
    /// â€” a US-style sheet would need its dates as real dates or ISO text.
    /// </para>
    /// </summary>
    ...
    /// <summary>Absent column and blank cell are the same thing here â€” see the class remarks.</summary>
    ...
    /// as currency or a percentage â€” the value is still the number underneath.
    /// </summary>
    private static string Clean(string raw) =>
        raw.Replace("â‚¹", string.Empty, StringComparison.Ordinal)
