...
/// <para>
/// Constants because endpoints reference them â€” <c>.RequirePermission(MastersPermissions.PartCreate)</c>
/// â€” and a typo must be a compile error rather than an endpoint nobody can reach.
/// </para>
/// <para>
...
/// can approve parts" â€” it says "approving a part requires this permission", and an
...
public static class MastersPermissions
{
    ...
    /// <summary>
    /// Sees the pay columns â€” PF, ESI, PT, TDS, gross, net and hourly rate.
    /// <para>
    ...
    /// could find a phone number could read the payroll â€” and the same grid also
    ...
    /// withholds the fields from its sort and filter map â€” otherwise
    ...
    /// <summary>
    /// Reads every business unit, not just the caller's own â€” the business unit table
    ...
    /// sub-assemblies the ability to restructure the section list â€” which is what a
    /// single <c>masters.assembly.*</c> permission would do â€” hands out an authority
    /// nobody asked for.
    /// </para>
    /// </summary>
    ...
    /// <summary>
    /// Maintains the code lists every other master picks from â€” lookup values,
    ...
    /// </para>
    /// <para>
    /// Separate from reading a list, which needs no permission at all â€” every form
