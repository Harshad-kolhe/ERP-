...
/// <summary>
/// A part master record â€” the atom the whole system is built on. Every BOM line,
...
public sealed class Part : AggregateRoot<PartId>, IAuditable, ISoftDeletable, IBusinessUnitScoped, IHasRowVersion
{
    ...
    /// This is the legacy <c>SysPartNumber</c> â€” the number the system issues. The
    /// manually assigned code lives in <see cref="ItemNumber"/>.
    /// </summary>
    public string PartNumber { get; private set; } = null!;

    /// <summary>
    /// The part number this one is a revision of â€” the legacy <c>OriginalPartNumber</c>.
    /// <para>
    ...
    /// <para>
    /// Separate from <see cref="Status"/>, and deliberately so â€” the two answer
    ...
    /// alone â€” the legacy screen fills most of these in later, and requiring them up
    ...
    /// number its family was issued under. Left null â€” every path except import â€”
    ...
    /// <summary>
    /// Pauses an approved part â€” legacy status <c>10</c>. It stays approved work,
    ...
    /// quietly loosened for everyone â€” which is what adding an <c>isImport</c> flag
    ...
    /// <para>
    /// Codes are upper-cased for the same reason part numbers are â€” see
    ...
    /// cannot become three different parts â€” a duplicate-master problem that is
