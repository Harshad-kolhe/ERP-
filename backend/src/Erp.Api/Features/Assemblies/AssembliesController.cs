...
[ApiController]
[Route("api/v1/masters/assemblies")]
[Tags("Masters")]
...
{
    private const AssemblyLevel Level = AssemblyLevel.Assembly;

    [HttpGet(Name = "ListAssemblies")]
    [RequirePermission(MastersPermissions.AssemblyRead)]
    [EndpointSummary("List assemblys")]
    [EndpointDescription(
    ...
    [HttpGet("{id:guid}", Name = "GetAssemblyById")]
    [RequirePermission(MastersPermissions.AssemblyRead)]
    [EndpointSummary("Get one assembly")]
    [EndpointDescription(
    ...
    [HttpPost(Name = "CreateAssembly")]
    [RequirePermission(MastersPermissions.AssemblyCreate)]
    [EndpointSummary("Create a assembly")]
    [EndpointDescription(
        ...
        CancellationToken cancellationToken)
    {
        ...
        return result.ToHttpResult(id => Results.Created($"/api/v1/masters/assemblies/{id}", new { id }));
    }

    [HttpPut("{id:guid}", Name = "UpdateAssembly")]
    [RequirePermission(MastersPermissions.AssemblyUpdate)]
    [EndpointSummary("Update a assembly")]
    [EndpointDescription(
