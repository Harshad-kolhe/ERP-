...
{
    private const AssemblyLevel Level = AssemblyLevel.Section;

    [HttpGet(Name = "ListSections")]
    [RequirePermission(MastersPermissions.SectionRead)]
    [EndpointSummary("List sections")]
    [EndpointDescription(
    ...
    [HttpGet("{id:guid}", Name = "GetSectionById")]
    [RequirePermission(MastersPermissions.SectionRead)]
    [EndpointSummary("Get one section")]
    [EndpointDescription(
    ...
    [RequirePermission(MastersPermissions.SectionCreate)]
    [EndpointSummary("Create a section")]
    [EndpointDescription(
        ...
        CancellationToken cancellationToken)
    {
        ...
        return result.ToHttpResult(id => Results.Created($"/api/v1/masters/sections/{id}", new { id }));
    }

    [HttpPut("{id:guid}", Name = "UpdateSection")]
    [RequirePermission(MastersPermissions.SectionUpdate)]
    [EndpointSummary("Update a section")]
    [EndpointDescription(
