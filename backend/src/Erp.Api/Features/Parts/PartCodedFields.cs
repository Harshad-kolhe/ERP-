...
/// one comes from.
/// </summary>
/// <param name="LookupType">Which master answers for this field â€” see <see cref="LookupTypes"/>.</param>
/// <param name="Column">
...
public sealed record PartCodedField(
    string LookupType,
    ImportColumn Column,
    Func<PartCodes, string?> Read);

/// <summary>The three places a part's codes arrive from over HTTP.</summary>
public sealed record PartCodes(string? UnitOfMeasureCode, string? HsnCode, PartAttributesDto? Attributes);
...
/// <c>TechnicalSpecification</c> is not â€” the test is whether a master answers for
/// the value, not whether it is short.
/// </para>
/// </summary>
public static class PartCodedFields
{
    ...
    /// <summary>The distinct masters these fields draw on â€” what a check has to load.</summary>
