...
namespace Erp.Persistence.Domain.UnitsOfMeasure;
...
/// </para>
/// <para>
/// <see cref="ConversionToBase"/> — a part bought in TON and stocked in KG is one
...
/// </para>
/// <para>
/// <see cref="Decimals"/> — 0.5 NOS is not a quantity. Precision belongs to the
...
{
    /// <summary>What gets stored on the record that references it — <c>KG</c>, <c>NOS</c>.</summary>
    ...
    /// part and 50 for another is a fact about the part, not about the box — see the
    ...
    /// there is no factor from KG to MTR, and returning the number unchanged — which
    /// is what a conversion helper that swallows the mismatch does — would put a
    ...
    public static Result<decimal> ConvertQuantity(UnitOfMeasure from, UnitOfMeasure to, decimal quantity)
    {
        ...
        if (!string.Equals(from.BaseCode, to.BaseCode, StringComparison.OrdinalIgnoreCase))
        {
            return Result.Failure<decimal>(Error.Validation(
                "uom.not_convertible",
                $"There is no conversion from '{from.Code}' to '{to.Code}' — they measure different things."));
        }
