...
/// <summary>The uploaded file, as the application layer sees it — no <c>IFormFile</c>.</summary>
internal sealed record ImportFile(Stream Content, string? FileName, long Length);
...
/// </summary>
internal static class MasterImportEndpointBuilder
{
    /// <summary>
    ...
    /// codebase — a hidden dependency is how a legacy controller reached 41
    ...
    /// <c>200</c> with a failure flag inside — that shape is exactly what this
        ...
        Func<ImportFile, TCommand> toCommand)
    {
        ...
        group.MapPost($"/{resource}/import", async (
                ...
                CancellationToken cancellationToken) =>
            {
                ...
                if (result.IsFailure)
                {
                    // The file never got as far as being read row by row — wrong
