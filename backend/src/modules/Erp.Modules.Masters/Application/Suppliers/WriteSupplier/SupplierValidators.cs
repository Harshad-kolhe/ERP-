...
/// <summary>
/// The rules for a supplier's editable fields, shared by create and update.
/// <para>
/// Lengths mirror <c>SupplierConfiguration</c> exactly. That is what turns an
/// over-long value into a message naming the field, instead of a SQL truncation
...
internal sealed class SaveSupplierValidator : AbstractValidator<SaveSupplierRequest>
{
    public SaveSupplierValidator()
    {
        ...
        // IFSC: four letters, a zero, then six alphanumerics. Wrong here means a
        // payment that bounces days later with no explanation attached to it.
...
internal sealed class CreateSupplierValidator : AbstractValidator<CreateSupplierRequest>
{
    public CreateSupplierValidator()
    {
        // Included rather than restated, so the two entry points cannot disagree
        // about what a valid supplier looks like.
...
internal sealed class UpdateSupplierValidator : AbstractValidator<UpdateSupplierRequest>
{
