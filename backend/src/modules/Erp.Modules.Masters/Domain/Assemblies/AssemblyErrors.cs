...
/// </para>
/// </summary>
internal static class AssemblyErrors
{
    ...
    // a node cannot become its own ancestor — the check would be a guard that can
    ...
    public static Error HasActiveChildren(int childCount) => Error.Conflict(
        ...
        $"This node still has {childCount} active child node(s). Deactivate or move them first — "
