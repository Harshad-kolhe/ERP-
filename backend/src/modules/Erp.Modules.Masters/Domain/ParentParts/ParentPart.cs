...
/// <c>AssemblyPartMaster</c> — two tables holding the same facts with nothing
...
/// the lines change — so they cannot be wrong and cannot be posted from a browser.
/// </para>
/// </summary>
internal sealed class ParentPart
    : AggregateRoot<ParentPartId>, IAuditable, ISoftDeletable, IBusinessUnitScoped, IHasRowVersion
{
    ...
    /// <see cref="AssemblyNode"/> and is optional — a bought-out sub-assembly need
    ...
    /// <summary>The component lines, in line-number order. Replaced wholesale — see <see cref="ReplaceComponents"/>.</summary>
    ...
    /// already, is checked by the handler — the only layer that can see other rows.
    /// </summary>
    ...
    /// The rows are few — a build is tens of lines, not thousands — so deleting and
...
/// </para>
/// </summary>
internal sealed record ParentPartComponentDraft(
    PartId PartId,
    decimal Quantity,
    string? UnitOfMeasureCode,
