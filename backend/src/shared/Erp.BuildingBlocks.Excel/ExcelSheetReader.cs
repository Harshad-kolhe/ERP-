...
public static class ExcelSheetReader
{
    ...
    /// <para>
    /// Failures here are about the <em>file</em> — unreadable, empty, too large,
    ...
    public static Result<ExcelSheet> Read(Stream stream)
    {
        ...
        using (workbook)
        {
            ...
            // Excel's used range routinely extends past the last real row — a cleared
            // cell still counts as used — so the cap is checked against rows that
        ...
        IReadOnlyList<string> headers)
    {
        ...
        for (var index = 0; index < headers.Count; index++)
        {
            ...
            // author's Excel used — that is how 03/04/2026 becomes two different days.
