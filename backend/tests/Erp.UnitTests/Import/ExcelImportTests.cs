...
namespace Erp.UnitTests.Import;
...
/// that goes wrong with an import is a property of actual spreadsheets â€” a header
...
public sealed class ExcelImportTests
{
    ...
    public void Row_numbers_are_the_ones_excel_shows()
    {
        ...
        // Row 1 is the header, so the first data row is 2 â€” the number the operator
    ...
    /// all the way across is skipped rather than reported â€” see
    /// <see cref="Blank_rows_are_skipped_rather_than_rejected"/> â€” so a row with
    ...
    /// <summary>
    /// An optional column absent from the sheet is not an error â€” an operator may
